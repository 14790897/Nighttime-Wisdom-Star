...
def receive_message():
    ...
    socketio.emit('remain_counts', {'remain_counts': counts}, room=username)
...
def init_chat():
    return jsonify(start_time=os.environ.get('start_time'), end_time=os.environ.get('end_time'), availableChats=r.get('remain_counts_key'))
